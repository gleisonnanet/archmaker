...
distroversion=""
distrocodename=""
echo "Bem-vindo ao script de geração de 'DISTNAME'"
echo "==========="
if [ "$1" != "" ]; then
  distroversion="$1"
  distrocodename="$2"
else
  echo -n "Por favor, insira a versão atual do  'DISTNAME' > "
  read distroversion
  echo -n "Por favor, insira o codinome atual de 'DISTNAME' > "
  read distrocodename
fi
createdir() {
...
compilecalamares() {
  ...
  echo "Preparando compilação do Calamares ... "
  # curl http://archmaker.guidedlinux.org/PKGBUILD > customrepo/archmaker-calamares/PKGBUILD
  curl https://raw.githubusercontent.com/Cleanjaro/calamares/master/PKGBUILD > customrepo/archmaker-calamares/PKGBUILD
...
setuprepo() {
  ...
  echo "Adicionando pacotes ao repositório ... "
...
cleanup() {
  ...
  echo "Excluindo arquivos  $finalfiles..."
   sudo rm -rf $finalfiles
   rm -rf ./customrepo
  echo "Salvando arquivo iso ... "
