...
ApplicationWindow {
    ...
    SwipeView {
        ...
        onCurrentIndexChanged: {
            if (swipeView.currentIndex == 6) {
                ...
                if (pg4.radioempty.checked) skelfolderoption = 0; skelfoldertext = qsTr("Vazio  /etc/skel-folder");
                if (pg4.radiohome.checked) skelfolderoption = 2; skelfoldertext = qsTr("Configuração de ~ as  /etc/skel");
                if (pg4.radiocustom.checked) {
                    if (pg4.skelfolderselection.text !== qsTr("No folder selected.")) {
                        ...
                        skelfolderoption = 1; skelfoldertext = qsTr("Personalizado /etc/skel-folder: ") + skelfolderpath;
                    } else {
                        skelfolderoption = 0; skelfoldertext = qsTr("Vazio  /etc/skel-folder");
                    }
                }
                ...
                if (pg6.radiodefault.checked) customslideshow = false; customslideshowtext = qsTr("Usar apresentação de slides padrão ");
                if (pg6.radiocustom.checked) {
                    if (pg6.slideshowselection.text !== qsTr("No folder selected.")) {
                        ...
                        customslideshow = true; customslideshowtext = qsTr("Usar apresentação de slides personalizada : ") + slideshowpath;
                    } else {
                        customslideshow = false; customslideshowtext = qsTr("Usar apresentação de slides padrão ");
                    }
                }
    ...
    footer: TabBar {
        ...
        TabButton {
            text: qsTr("Programas  (repos)")
        }
        TabButton {
            text: qsTr("Programas  (AUR)")
        }
        TabButton {
